...
(() => {
  ...
  class HeatLabScene extends HTMLElement {
    static get observedAttributes() { return ['experiment', 'view', 'target', 'tool', 'action']; }

    constructor() {
      super();
      this.cfg = { experiment: 'solid', view: 'both', target: T_ROOM, tool: 'thermometer', action: '' };
      this.temp = T_ROOM;
    ...
    attributeChangedCallback(n, o, v) {
      if (n === 'target') this.cfg.target = clamp(parseFloat(v) || T_ROOM, 0, 260);
      else if (n === 'action') {
    ...
    buildApparatus() {
      ...
      const bench = new T.Mesh(new T.CylinderGeometry(0.44, 0.44, 0.012, 64), new T.MeshStandardMaterial({ color: 0x091521, roughness: 1 }));
      bench.name = 'bench';
    ...
    buildGas() {
      ...
      const balloon = new T.Mesh(new T.SphereGeometry(1, 40, 30), M.rubber);
      ...
      const burner = this.burner(); g.add(burner);
      g.userData = { balloon, frags, burner, neckY: 0.372 };
      return g;
    }
    ...
    frame(ms) {
      ...
      Object.keys(this.rigs).forEach(k => { this.rigs[k].visible = k === exp; });
      ...
      // cameras
      const focus = { solid: [0, 0.20, 0.62], liquid: [0, 0.26, 0.8], gas: [0, 0.25, 0.8], feel: [0, 0.09, 0.66] }[exp] || [0, 0.2, 0.8];
      if (this._lastExp !== exp) { this._lastExp = exp; this.cam.dist = focus[2]; }
      const ty = focus[1];
    ...
    updSolid(ph, dt, now) {
      ...
      if (this.test) {
        ...
        else if (icing) { basinX = 0; ballY = 0.072; }
      }
      a.ringX = lerp(a.ringX, ringX, 1 - Math.pow(0.02, dt));
      a.burnerX = lerp(a.burnerX, burnerX, 1 - Math.pow(0.03, dt));
      a.basinX = lerp(a.basinX, basinX, 1 - Math.pow(0.03, dt));
      a.ballY = lerp(a.ballY, ballY, 1 - Math.pow(0.02, dt));
      ...
      flame.visible = heating && !this.test;
      if (flame.visible) flame.scale.setScalar(0.94 + Math.sin(now * 22) * 0.06);
      u.basin.userData.water.material = icing ? this.mats.cool : this.mats.cool;
    ...
    updLiquid(ph, now) {
      ...
      u.basin.userData.water.material = hot ? this.mats.hot : this.mats.cool;
      u.basin.userData.ice.visible = icing;
      u.steam.visible = hot;
    ...
    updGas(ph, now, dt) {
      ...
      if (this.popped) {
        ...
        if (ph.balloonMl > 0.5) {
          ...
          const r = Math.max(0.014, (ph.balloonCm / 2) * 0.01) * strain;
          b.scale.set(r, r * 0.95, r);
          b.position.y = u.neckY + r * 0.85;
        } else {
          const limp = 0.017 + ph.suckMl * 0.00004;
          b.scale.set(limp, limp * 0.5, limp);
          b.position.y = u.neckY + 0.004;
        }
        if (ph.balloonMl > BURST_ML) {
          ...
          if (this.burstT >= 3) {
            ...
            frags.children.forEach((p, i) => {
              ...
              p.position.set(Math.cos(a) * r * 0.8, u.neckY + r * 0.9, Math.sin(a) * r * 0.8);
